...
# TODO: Can make this file work on all the LiveJournal datasets instead of one.
#  Roughly same approach as comparison_cd_algs.py
...
def evaluate_method(config):
    ...
    with open(
        ...
        print(f"Read graph and starting predictions for method {cd_method}")
        ...
        try:
            ...
            print(f"Exception {repr(e)} for method {cd_method}")
            emd.append(None)
            f1.append(None)
            fcc.append(None)
            ari.append(None)
        ...
        print(f"prediction completed for method {cd_method}")
        ...
        print(f"fairness metrics calculated for method {cd_method}")
    ...
    try:
        with open(
            f"results/LiveJournal-overlapping-processed/{fairness_type}-{cd_method.__name__}-fairness-scores.pickle",
            "wb",
        ) as handle:
            pickle.dump(fairness_scores, handle, protocol=pickle.HIGHEST_PROTOCOL)

        with open(
            f"results/LiveJournal-overlapping-processed/{fairness_type}-{cd_method.__name__}-evaluation-scores.pickle",
            "wb",
        ) as handle:
        ...
        with open(
            f"results/LiveJournal-overlapping-processed/{fairness_type}-{cd_method}-fairness-scores.pickle",
            "wb",
        ) as handle:
            pickle.dump(fairness_scores, handle, protocol=pickle.HIGHEST_PROTOCOL)

        with open(
            f"results/LiveJournal-overlapping-processed/{fairness_type}-{cd_method}-evaluation-scores.pickle",
            "wb",
        ) as handle:
            pickle.dump(evaluation_scores, handle, protocol=pickle.HIGHEST_PROTOCOL)
